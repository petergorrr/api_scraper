...
if os.path.isdir(f"TFT_summoners_match_data_{region}") is False:
    os.mkdir(f"TFT_summoners_match_data_{region}")
...
for path in pathlib.Path(f"TFT_match_id_{region}").iterdir():
    ...
    for match_id in data:
        if os.path.isdir(f"TFT_summoners_match_data_{region}/matches_{puuid}"):
            print("The match folder already exists.")

        else:
            os.mkdir(f"TFT_summoners_match_data_{region}/matches_{puuid}")
        ...
        with open(f"TFT_summoners_match_data_{region}/matches_{puuid}/{match_id}_match.json", "w", encoding='utf-8')as f:
